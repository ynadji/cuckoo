...
def startgame(game, ip):
    ...
    os.system('iptables -t %s -I %s -d %s %s -m %s -p %s -j NFQUEUE --queue-num %s'
            % (table, chain, ip, source, transport, transport, vmnum))
    main(arglist=args)

def stopgame(game, ip):
    vmnum = ip.split('.')[2]
    if game.startswith('dns'):
        transport = 'udp'
        table = 'filter'
    ...
    os.system('iptables -t %s -D %s -d %s %s -m %s -p %s -j NFQUEUE --queue-num %s'
            % (table, chain, ip, source, transport, transport, vmnum))
